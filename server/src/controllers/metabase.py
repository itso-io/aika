...
def createMetabase(aika_user, local=False):
    ...
    newuser_first = aika_user.given_name
    newuser_last = aika_user.family_name
    newuser_email = randomString() + '_' + aika_user.email

    # There is a password generator in the Utils
    pwd = random_password()

    metabase_user = UserMetabase(
        user_id=aika_user.id,
        email=newuser_email,
        password=pwd
    )
